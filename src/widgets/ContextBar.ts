...
import { makePacmanProgressBar } from './shared/progress-bar';
import { makeSliderBar } from './shared/usage-display';

type DisplayMode = 'pacman' | 'progress' | 'progress-short' | 'slider' | 'slider-only';

function getDisplayMode(item: WidgetItem): DisplayMode {
    const mode = item.metadata?.display;
    if (mode === 'pacman' || mode === 'progress' || mode === 'slider' || mode === 'slider-only') {
        return mode;
    }
...
function getPacmanWidth(item: WidgetItem): number {
    const rawWidth = Number(item.metadata?.width);
    if (!Number.isFinite(rawWidth)) {
        return 18;
    }
    return Math.max(8, Math.min(40, Math.trunc(rawWidth)));
}

function isPacmanAscii(item: WidgetItem): boolean {
    return item.metadata?.ascii === 'true';
}

function makePacmanDisplay(percent: number, item: WidgetItem): string {
    return `[${makePacmanProgressBar(percent, getPacmanWidth(item), { ascii: isPacmanAscii(item) })}]`;
}
...
export class ContextBarWidget implements Widget {
    ...
    render(item: WidgetItem, context: RenderContext, settings: Settings): string | null {
        ...
        if (context.isPreview) {
            if (displayMode === 'pacman') {
                const pacmanDisplay = `${makePacmanDisplay(25, item)} 50k/200k (25%)`;
                return item.rawValue ? pacmanDisplay : `Context: ${pacmanDisplay}`;
            }
        ...
        if (displayMode === 'pacman') {
            const pacmanDisplay = `${makePacmanDisplay(clampedPercent, item)} ${usedK}k/${totalK}k (${Math.round(clampedPercent)}%)`;
            return item.rawValue ? pacmanDisplay : `Context: ${pacmanDisplay}`;
        }
