...
describe('ContextBarWidget', () => {
    ...
    it('renders pac-man progress bar mode when configured', () => {
        const context: RenderContext = {
            data: {
                context_window: {
                    context_window_size: 200000,
        ...
        expect(widget.render({
            id: 'ctx',
            type: 'context-bar',
            metadata: { display: 'pacman', width: '8' }
        }, context, DEFAULT_SETTINGS)).toBe('Context: [ ᗧ••••••] 30k/200k (15%)');
    });
    ...
    it('cycles display modes in the expected order', () => {
        ...
        const fifth = widget.handleEditorAction('toggle-progress', fourth ?? base);

        expect(first?.metadata?.display).toBe('progress');
        expect(second?.metadata?.display).toBe('pacman');
        expect(third?.metadata?.display).toBe('slider');
        expect(fourth?.metadata?.display).toBe('slider-only');
        expect(fifth?.metadata?.display).toBe('progress-short');
    });
});
