...
describe('makeTimerProgressBar', () => {
    ...
    describe('pac-man bar', () => {
        it('renders pac-man at the start for 0%', () => {
            expect(makePacmanProgressBar(0, 8)).toBe('ᗧ•••••••');
        });

        it('moves pac-man through eaten pellets', () => {
            expect(makePacmanProgressBar(50, 8)).toBe('    ᗧ•••');
        });

        it('renders pac-man at the end for 100%', () => {
            expect(makePacmanProgressBar(100, 8)).toBe('       ᗧ');
        });

        it('supports ASCII fallback', () => {
            expect(makePacmanProgressBar(50, 8, { ascii: true })).toBe('    C...');
        });

        it('clamps percent and width', () => {
            expect(makePacmanProgressBar(150, 1)).toBe('ᗧ');
            expect(makePacmanProgressBar(-10, 4)).toBe('ᗧ•••');
        });
    });
});
