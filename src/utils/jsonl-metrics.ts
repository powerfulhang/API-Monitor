...
export async function getTokenMetrics(transcriptPath: string): Promise<TokenMetrics> {
    try {
        ...
        if (!fs.existsSync(transcriptPath)) {
            return { inputTokens: 0, outputTokens: 0, cachedTokens: 0, totalTokens: 0, contextLength: 0, apiCallCount: 0 };
        }
        ...
        const apiCallCount = entriesToCount.length;

        return { inputTokens, outputTokens, cachedTokens, totalTokens, contextLength, apiCallCount };
    } catch {
        return { inputTokens: 0, outputTokens: 0, cachedTokens: 0, totalTokens: 0, contextLength: 0, apiCallCount: 0 };
    }
}
