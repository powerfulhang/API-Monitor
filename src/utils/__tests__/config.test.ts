...
function getSettingsPaths(): { configDir: string; settingsPath: string; backupPath: string } {
    const configDir = path.join(mockHomeDir, '.config', 'ccstatusline');
    return {
        configDir,
...
function getClaudeConfigDir(): string {
    return path.join(mockHomeDir, '.claude');
}
...
describe('config utilities', () => {
    ...
    beforeEach(() => {
        mockHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ccstatusline-config-test-home-'));
