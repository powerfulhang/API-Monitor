...
function quoteExpectedConfigPath(filePath: string): string {
    const needsQuoting = process.platform === 'win32'
        ? /[\s&()<>|^"]/.test(filePath)
        : /[\s()[\];&#|'"\\$`]/.test(filePath);
...
function expectedInstalledCommand(baseCommand: string): string {
    return `${baseCommand} --config ${quoteExpectedConfigPath(getConfigPath())}`;
}
...
describe('buildCommand via installStatusLine', () => {
    ...
    it('should append --config with simple path (no quoting needed)', async () => {
        ...
        await installStatusLine(false);
        expect(readInstalledCommand()).toBe(expectedInstalledCommand(CCSTATUSLINE_COMMANDS.NPM));
    });
    ...
    it('should use bunx command when useBunx is true', async () => {
        ...
        expect(readInstalledCommand()).toBe(expectedInstalledCommand(CCSTATUSLINE_COMMANDS.BUNX));
    });
