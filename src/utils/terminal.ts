...
let execSyncForTerminal = childProcess.execSync;

export function setTerminalExecSyncForTests(execSync: typeof childProcess.execSync | null): void {
    execSyncForTerminal = execSync ?? childProcess.execSync;
}
...
function probeTerminalWidth(): number | null {
    ...
    const platform = process.env.CCSTATUSLINE_TEST_TERMINAL_PLATFORM ?? process.platform;
    if (platform === 'win32') {
        return null;
    }
    ...
    try {
        const width = execSyncForTerminal('tput cols 2>/dev/null', {
            encoding: 'utf8',
...
function getParentProcessId(pid: number): number | null {
    try {
        const parentPidOutput = execSyncForTerminal(`ps -o ppid= -p ${pid}`, {
            encoding: 'utf8',
            stdio: ['pipe', 'pipe', 'ignore'],
            shell: '/bin/sh'
        }).trim();
...
function getTTYForProcess(pid: number): string | null {
    try {
        const tty = execSyncForTerminal(`ps -o tty= -p ${pid}`, {
            encoding: 'utf8',
            stdio: ['pipe', 'pipe', 'ignore'],
            shell: '/bin/sh'
        }).replace(/\s+/g, '');
...
function getWidthForTTY(tty: string): number | null {
    try {
        const width = execSyncForTerminal(
