...
**🎨 A Claude Code status line formatter tuned for API monitoring**
*Display model info, Pac-Man context progress, API credit usage, API call counts, git state, and session timing in your terminal*
...
[![Repository](https://img.shields.io/badge/GitHub-powerfulhang%2FAPI--Monitor-blue)](https://github.com/powerfulhang/API-Monitor)

[![Mentioned in Awesome Claude Code](https://awesome.re/mentioned-badge.svg)](https://github.com/hesreallyhim/awesome-claude-code)
[![ClaudeLog - A comprehensive knowledge base for Claude](https://claudelog.com/img/claude_log_badge.svg)](https://claudelog.com/)


This project is an API-monitoring-focused fork of [sirmalloc/ccstatusline](https://github.com/sirmalloc/ccstatusline). Original authorship and license notices are retained in [AUTHORS](AUTHORS) and [NOTICE](NOTICE).
...
### API Monitor fork - Pac-Man context and CNY API usage

- **Pac-Man context bar** - `Context Bar` can render a compact Pac-Man style progress bar for context usage.
- **Default API monitor layout** - Sensible default line now includes model, Pac-Man context, API credits, API calls, git state, and session time.
- **CNY API credit display** - API credits render as ASCII-safe `CNY` text to avoid terminal encoding glitches.

### v2.3.0 - Multi-provider API Credit & API Call Count widgets

- **💳 Multi-provider API Credit widget** - Added `API Credits` widget that automatically detects the current AI provider (MiMo, DeepSeek, etc.) and calculates credit consumption in CNY.
  - Supports MiMo models: mimo-v2.5-pro, mimo-v2-pro, mimo-v2.5, mimo-v2-omni, mimo-v2-flash
  - Supports DeepSeek models: deepseek-v4-pro, deepseek-v4-flash (with cache-hit pricing)
  - Extensible provider registry for adding more AI providers
  - Shows provider name, credit amount, and token breakdown (input/output/cached)
- **📊 API Call Count widget** - Added `API Calls` widget that tracks the number of API calls in the current session from transcript data.
- **🔄 Backward compatibility** - Old `mimo-credit` widget type automatically maps to new `api-credit` type.
...
- **ᗧ Pac-Man Context Progress** - Show context usage as an at-a-glance Pac-Man progress bar
- **💳 Multi-provider API Credits** - Automatically detect AI provider (MiMo, DeepSeek) and calculate credit consumption in CNY with cache-hit pricing support
- **📊 API Call Tracking** - Track the number of API calls in your current session
